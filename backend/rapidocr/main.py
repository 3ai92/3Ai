...
def parse_args(arg_list: Optional[List[str]] = None):
    ...
    parser.add_argument("-markdown", "--to_markdown", action="store_true", default=False, help="Export result to markdown file.")
    parser.add_argument("-json", "--to_json", action="store_true", default=False, help="Export result to JSON file.")
...
def main(arg_list: Optional[List[str]] = None):
    ...
    if args.to_markdown:
        ...
        md_path = Path(args.img_path).with_suffix('.md')
        with open(md_path, "w", encoding="utf-8") as f:
            f.write(markdown_str)
        print(f"Markdown result has been saved in {md_path}")
    ...
    if args.to_json:
        ...
        json_path = Path(args.img_path).with_suffix('.json')
        with open(json_path, "w", encoding="utf-8") as f:
            f.write(json_str)
        print(f"JSON result has been saved in {json_path}")
